...
public class TicketController {
	...
	public ResponseEntity<?> createTicketGrantingTicket(
		...
		if (re != null && re.getBody() != null) {
			...
			// 2. 将成功返回的TGC添加cookie
			// toceanCookieGenerator.addCookie(tgtId);
	...
	public ResponseEntity<?> createTicketGrantingTicketThruTGCCookie(
		...
		for (Cookie c : cs) {
			LOGGER.debug("name:" + c.getName() + "****value:" + c.getValue());
		}
		...
		// LOGGER.debug("tgtId:" + tgtId);
		// }
		LOGGER.debug("createTicketGrantingTicketThruTGCCookie->tgtId:" + tgtId);
		ResponseEntity<?> re = null;
		if (tgtId != null && !StringUtils.isBlank(tgtId)) {
			re = serviceTicketResourceRestController.createServiceTicket(request, tgtId);
			if (re != null) {
	...
	private void printParam(final HttpServletRequest request) {
		Enumeration enu = request.getParameterNames();
		while (enu.hasMoreElements()) {
			String paraName = (String) enu.nextElement();
			LOGGER.debug("Parameter-" + paraName + ": " + request.getParameter(paraName));
		}
	}

	private void printHeader(final HttpServletRequest request) {
		// 获取所有的消息头名称
		Enumeration<String> headerNames = request.getHeaderNames();
		// 获取获取的消息头名称，获取对应的值，并输出
		while (headerNames.hasMoreElements()) {
			String nextElement = headerNames.nextElement();
			LOGGER.debug("Header-" + nextElement + ":" + request.getHeader(nextElement));
		}
